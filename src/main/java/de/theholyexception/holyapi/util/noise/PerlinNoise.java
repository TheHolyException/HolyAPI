package de.theholyexception.holyapi.util.noise;

import java.util.Random;

public class PerlinNoise {

    private static final int[] DEFAULT = new int[512];
    private static final int[] DEFAULT_PERMUTATION = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225,
    ...
    static {
        for (int i = 0; i < 256; i++) {
            DEFAULT[256 + i] = DEFAULT[i] = DEFAULT_PERMUTATION[i];
        }
    }

    private int[] p = DEFAULT.clone();
    public PerlinNoise(){}
    ...
    public PerlinNoise(long seed){
        ...
        for (int i = 0; i < 512; i++) {
            p[i] = random.nextInt(255);
        }
    }

    public static float improvedNoise(float x, float y, float z) {
        return new PerlinNoise().improvedNoise0(x, y, z);
    }

    public PerlinNoise(int seed) {
        Random random = new Random(seed);
        for (int i = 0; i < 512; i++) p[i] = random.nextInt(255);
    }

    /**
     * PerlinNoise using three axis (0.0 - 255.0).
     * If a axis is smaller or bigger than 0-255 than it will cut them down.
     * @param x
     * @param y
     * @param z
     * @return noise-value at (x|y|z) (0.0 - 1.0)
     */
    public float improvedNoise0(float x, float y, float z) {
